...
| **Sprint 2** | **Phase 1** | **Filtrage iOS CallKit + Sortants + Modes** ⭐ | **✅** | **en cours** |
...
### Sprint 2 — Filtrage iOS CallKit + Appels sortants + Modes prédéfinis ⭐ TERMINÉ

**Date :** 24/05/2026 · **Tag :** `v1.1.0-sprint-2`

#### User stories réalisées

| ID | Story | Statut |
|---|---|---|
| US-2-001 | HarmonyCallDirectoryExtension (Swift CXCallDirectoryProvider) | ✅ |
| US-2-002 | SharedRulesStore — UserDefaults partagés via App Group | ✅ |
| US-2-003 | AppDelegate MethodChannel iOS (`com.harmony.app/call_filter_ios`) | ✅ |
| US-2-004 | CallFilterChannel.dart cross-platform (Android + iOS dispatch) | ✅ |
| US-2-005 | OutgoingCallDetector — FR surtaxés + US premium + international | ✅ |
| US-2-006 | OutgoingCallAlertDialog — warning amber/rouge, coût estimé | ✅ |
| US-2-007 | HarmonyFilterMode (6 modes) + FilterModeManager + kHarmonyFilterModes | ✅ |
| US-2-008 | Kotlin : WORK + WEEKEND ajoutés à FilterMode.kt + CallDecisionEngine | ✅ |
| US-2-009 | 15 clés i18n × 5 langues (iOS filtrage, alertes sortants, modes) | ✅ |
| US-2-010 | Tests Flutter 95/95, Kotlin 9/9, XCTest 4 (Xcode uniquement) | ✅ |

#### Bilan

| Métrique | Valeur |
|---|---|
| Tests Flutter | **95 verts** (+30 nouveaux) |
| Tests Kotlin JUnit | **9 verts** (+4 nouveaux) |
| Tests XCTest Swift | **4 écrits** (run en Xcode) |
| Issues flutter analyze | **0** |
| Fichiers Dart créés | 4 (detector, dialog, channel, manager) |
| Fichiers Swift créés | 4 (handler, store, tests, Info.plist) |
| Clés i18n ajoutées | 9 × 5 langues |
| Tag | **`v1.1.0-sprint-2`** |

#### Limitations documentées (honnêteté technique)

- `CXCallDirectoryProvider` = liste statique rechargée via `reloadExtension`, pas temps-réel
- Les appels sortants sont détectés côté Flutter uniquement (iOS CallKit n'expose pas d'API tierce pour les sortants)
- L'extension doit être activée manuellement dans Réglages → Téléphone → Blocage et identification d'appels
- XCTest nécessite Xcode sur macOS — non exécutable en CLI Windows
...
**Vélocité moyenne 10 sprints : 13.2 pts/sprint** · **100% complétion**

---

## Prochaine étape — Sprint 3

**Géolocalisation + SOS + Dashboard enrichi** — démarrer quand prêt.
